 ...
 <!-- Кнопка Импорта Вопросов -->
    <a href="{{ route('admin.tests.import', $test->id) }}" class="btn btn-success mb-3">Импортировать Вопросы</a>
        ...
        <div class="form-group">
            ...
            <input type="text" name="title" id="title" class="form-control" required value="{{ old('title', $test->title) }}">
            @error('title')
        ...
        <div class="form-group">
            ...
            <select name="university_id" id="university_id" class="form-control" required>
                ...
                @foreach($universities as $university)
                    <option value="{{ $university->id }}" {{ (old('university_id', $test->university_id) == $university->id) ? 'selected' : '' }}>
                        {{ $university->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="group_id" id="group_id" class="form-control" required>
                ...
                @foreach($groups as $group)
                    <option value="{{ $group->id }}" {{ (old('group_id', $test->group_id) == $group->id) ? 'selected' : '' }}>
                        {{ $group->name }}
                    </option>
                @endforeach
            </select>
